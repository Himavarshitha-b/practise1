...
OptionalDouble
averagefemale=list.stream().filter((gender)->gender.getGender().contains("Female")).mapToDouble((age)->age.getAge()).average();
OptionalDouble
...
long productdevelop=list.stream().filter((product)->product.getDepartment().contains("ProductDevelopment")).count();
long s_m=list.stream().filter((sm)->sm.getDepartment().contains("Sales AndMarketing")).count();
long s_t=list.stream().filter((st)->st.getDepartment().contains("Security AndTransport")).count();
...
long a_f=list.stream().filter((af)->af.getDepartment().contains("Account AndFinance")).count();
Map<String,Long>count=new HashMap<>();
if(productdevelop>0)
...
list.add(new Candidate(111,"Damon Salvatore",23,"Male","ProductDevelopment",2009,70000));
list.add(new Candidate(222,"Elena Gilbert",25,"Female","ProductDevelopment",2012,50000));
list.add(new Candidate(333,"Stefan Salvatore",30,"Male","ProductDevelopment",2009,60000));
list.add(new Candidate(444,"Carolyn Forbes",26,"Female","ProductDevelopment",2010,65000));
